...
CLI_CXX_FILES := $(wildcard $(CLI_CODE)/*.cpp)
CLI_OBJS_BUILD_PATH := $(OBJS_BUILD_PATH)/cli
CLI_OBJS := $(addprefix $(CLI_OBJS_BUILD_PATH)/,$(patsubst %.cpp,%.o,$(notdir $(CLI_CXX_FILES))))
$(CLI_OBJS_BUILD_PATH)/%.o: $(CLI_CODE)/%.cpp
	@mkdir -p $(CLI_OBJS_BUILD_PATH)
	$(CXX) $(SHARED_CFLAGS) $(WARNING_FLAGS) -MMD -MP $(CLI_INCLUDES) -c $< -o $@
...
CLI_PROGRAM_NAME := map_edit_cli
$(EXE_BUILD_PATH)/$(CLI_PROGRAM_NAME) : $(CORE_OBJS) $(CLI_OBJS)
	@mkdir -p $(EXE_BUILD_PATH)
	$(CXX) $(CORE_OBJS) $(CLI_OBJS) $(CLI_INCLUDES) -o $@ $(SHARED_LDFLAGS) $(WARNING_FLAGS)
...
.PHONY: val_cli
val_cli: $(EXE_BUILD_PATH)/$(CLI_PROGRAM_NAME)
	valgrind ./$(EXE_BUILD_PATH)/$(CLI_PROGRAM_NAME)

.PHONY: run_cli
run_cli: $(EXE_BUILD_PATH)/$(CLI_PROGRAM_NAME)
	./$(EXE_BUILD_PATH)/$(CLI_PROGRAM_NAME)
