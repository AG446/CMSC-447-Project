...
$(EXE_BUILD_PATH)/$(TESTER_PROGRAM_NAME) : $(CORE_OBJS) $(TESTS_OBJS)
	...
	$(CC) $(CORE_OBJS) $(TESTS_OBJS) $(TESTER_INCLUDES) -o $@ $(SHARED_LDFLAGS) $(WARNING_FLAGS)

CLI_PROGRAM_NAME := map_edit_cli
$(EXE_BUILD_PATH)/$(CLI_PROGRAM_NAME) : $(CORE_OBJS) $(CLI_OBJS)
	@mkdir -p $(EXE_BUILD_PATH)
	$(CC) $(CORE_OBJS) $(CLI_OBJS) $(CLI_INCLUDES) -o $@ $(SHARED_LDFLAGS) $(WARNING_FLAGS)

UI_PROGRAM_NAME := navigator
$(EXE_BUILD_PATH)/$(UI_PROGRAM_NAME) : $(CORE_OBJS) $(UI_OBJS)
	@mkdir -p $(EXE_BUILD_PATH)
	$(CC) $(CORE_OBJS) $(UI_OBJS) $(UI_INCLUDES)  -o $@ $(SHARED_LDFLAGS) $(shell pkg-config --libs $(EXTERNAL_LIBRARIES)) $(WARNING_FLAGS)
